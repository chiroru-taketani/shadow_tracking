...
struct Connection_information::pImpl
{
...
#else
            device_or_ip_name_ = "/dev/tty.usbmodem14111";  //USB接続時のポート番号(ターミナルで番号確認)
#endif
        }
    ...
    void set_ethernet_connection(const char* ip_address = NULL)
    {
        ...
        if (ip_address != NULL) {
            ...
            device_or_ip_name_ = "192.168.0.15";  //LAN接続時のIPアドレス(センサに設定されている値)
        }
...
: pimpl(new pImpl)
{
    ...
    //USB接続かLAN接続かで以下のいずれかを選択
    pimpl->set_serial_connection();  //USB
    //pimpl->set_ethernet_connection();  //LAN
}
