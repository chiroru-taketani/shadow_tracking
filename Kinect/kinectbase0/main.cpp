...
cv::Mat depthImage, depthDispImage, binImage;

Vec_3D imagePos[GL_WIN_SIZE_X][GL_WIN_SIZE_Y];
Vec_3D imageCol[GL_WIN_SIZE_X][GL_WIN_SIZE_Y];
...
{
    //視点
    eDegY = 180.0; eDegX = 0.0; eDist = 1000.0;
    //目標点
    tg.x = 0.0; tg.y = 0.0; tg.z = 1000;
...
void display()
{
    ...
    for (int i=0; i<rtn; i++) {
        for (int j=0; j<jointNum; j++) {
            glPushMatrix();
            glTranslated(jointPos[i][j].X, jointPos[i][j].Y, jointPos[i][j].Z);  //人物iのj番目の関節点の座標
            glutSolidSphere(25.0, 18, 9);  //関節点に球を配置
            glPopMatrix();
        }
    }
    ...
    for (int i=0; i<rtn; i++) {
        glBegin(GL_LINES);
        //頭-首
        glVertex3d(jointPos[i][0].X, jointPos[i][0].Y, jointPos[i][0].Z);
        glVertex3d(jointPos[i][1].X, jointPos[i][1].Y, jointPos[i][1].Z);
        //首-腹
        glVertex3d(jointPos[i][1].X, jointPos[i][1].Y, jointPos[i][1].Z);
        glVertex3d(jointPos[i][2].X, jointPos[i][2].Y, jointPos[i][2].Z);
        //首-左肩
        glVertex3d(jointPos[i][1].X, jointPos[i][1].Y, jointPos[i][1].Z);
        glVertex3d(jointPos[i][3].X, jointPos[i][3].Y, jointPos[i][3].Z);
        //左肩-左肘
        glVertex3d(jointPos[i][3].X, jointPos[i][3].Y, jointPos[i][3].Z);
        glVertex3d(jointPos[i][4].X, jointPos[i][4].Y, jointPos[i][4].Z);
        //左肘-左手
        glVertex3d(jointPos[i][4].X, jointPos[i][4].Y, jointPos[i][4].Z);
        glVertex3d(jointPos[i][5].X, jointPos[i][5].Y, jointPos[i][5].Z);
        //首-右肩
        glVertex3d(jointPos[i][1].X, jointPos[i][1].Y, jointPos[i][1].Z);
        glVertex3d(jointPos[i][6].X, jointPos[i][6].Y, jointPos[i][6].Z);
        //右肩-右肘
        glVertex3d(jointPos[i][6].X, jointPos[i][6].Y, jointPos[i][6].Z);
        glVertex3d(jointPos[i][7].X, jointPos[i][7].Y, jointPos[i][7].Z);
        //右肘-右手
        glVertex3d(jointPos[i][7].X, jointPos[i][7].Y, jointPos[i][7].Z);
        glVertex3d(jointPos[i][8].X, jointPos[i][8].Y, jointPos[i][8].Z);
        //腹-左腰
        glVertex3d(jointPos[i][2].X, jointPos[i][2].Y, jointPos[i][2].Z);
        glVertex3d(jointPos[i][9].X, jointPos[i][9].Y, jointPos[i][9].Z);
        //左腰-左膝
        glVertex3d(jointPos[i][9].X, jointPos[i][9].Y, jointPos[i][9].Z);
        glVertex3d(jointPos[i][10].X, jointPos[i][10].Y, jointPos[i][10].Z);
        //左膝-左足
        glVertex3d(jointPos[i][10].X, jointPos[i][10].Y, jointPos[i][10].Z);
        glVertex3d(jointPos[i][11].X, jointPos[i][11].Y, jointPos[i][11].Z);
        //腹-右腰
        glVertex3d(jointPos[i][2].X, jointPos[i][2].Y, jointPos[i][2].Z);
        glVertex3d(jointPos[i][12].X, jointPos[i][12].Y, jointPos[i][12].Z);
        //右腰-右膝
        glVertex3d(jointPos[i][12].X, jointPos[i][12].Y, jointPos[i][12].Z);
        glVertex3d(jointPos[i][13].X, jointPos[i][13].Y, jointPos[i][13].Z);
        //右膝-右足
        glVertex3d(jointPos[i][13].X, jointPos[i][13].Y, jointPos[i][13].Z);
        glVertex3d(jointPos[i][14].X, jointPos[i][14].Y, jointPos[i][14].Z);
        //腹-左肩
        glVertex3d(jointPos[i][2].X, jointPos[i][2].Y, jointPos[i][2].Z);
        glVertex3d(jointPos[i][3].X, jointPos[i][3].Y, jointPos[i][3].Z);
        //腹-右肩
    ...
    //スキャン点"pointPos[GL_WIN_TOTAL]"，色"pointCol[GL_WIN_TOTAL]，ラベルpointLabel[GL_WIN_TOTAL]"取得
    getDepthImage();
...
//        if (pointLabel[i]==0)
//            glColor4d(pointCol[i].nRed/255.0, pointCol[i].nGreen/255.0, pointCol[i].nBlue/255.0, alpha);
//        else
...
//    glEnd();
    ...
    glPushMatrix();
    glTranslated(0.0, 0.0, 900.0);
    glScaled(2000.0, 2000.0, 1.0);
    glBegin(GL_QUADS);
    ...
    for (int i=0; i<GL_WIN_TOTAL; i++) {
        ...
        imagePos[M][N].x = pointPos[i].X;
        imagePos[M][N].y = pointPos[i].Y;
        imagePos[M][N].z = pointPos[i].Z;
        
        imageCol[M][N].x = pointCol[i].nRed/255.0;
        imageCol[M][N].y = pointCol[i].nGreen/255.0;
        imageCol[M][N].z = pointCol[i].nBlue/255.0;
    }

    glBegin(GL_QUADS);
    for (int j=0; j<GL_WIN_SIZE_Y-1; j++) {
        for (int i=0; i<GL_WIN_SIZE_X-1; i++) {
            //if (imagePos[i][j].z<100 || imagePos[i][j+1].z<100 || imagePos[i+1][j+1].z<100 || imagePos[i+1][j].z<100) continue;
            //if (imagePos[i][j].z>900 && imagePos[i][j+1].z>900 && imagePos[i+1][j+1].z>900 && imagePos[i+1][j].z>900) continue;
            
//            if (imagePos[i][j].z>901) imagePos[i][j].z = 901;
//            if (imagePos[i][j+1].z>901) imagePos[i][j+1].z = 901;
//            if (imagePos[i+1][j+1].z>901) imagePos[i+1][j+1].z = 901;
//            if (imagePos[i+1][j].z>901) imagePos[i+1][j].z = 901;

            //if (fabs(imagePos[i][j].z-imagePos[i][j+1].z)>100 || fabs(imagePos[i][j+1].z-imagePos[i+1][j+1].z)>100 || fabs(imagePos[i+1][j+1].z-imagePos[i+1][j].z)>100 || fabs(imagePos[i+1][j].z-imagePos[i][j].z)>100) continue;

            //glColor4d(1.0, 1.0, 1.0, 1.0);
            glColor4d(imageCol[i][j].x, imageCol[i][j].y, imageCol[i][j].z, 1.0);
            glVertex3d(imagePos[i][j].x, imagePos[i][j].y, imagePos[i][j].z);  //空間に点を配置
            glColor4d(imageCol[i][j+1].x, imageCol[i][j+1].y, imageCol[i][j+1].z, 1.0);
            glVertex3d(imagePos[i][j+1].x, imagePos[i][j+1].y, imagePos[i][j+1].z);  //空間に点を配置
            glColor4d(imageCol[i+1][j+1].x, imageCol[i+1][j+1].y, imageCol[i+1][j+1].z, 1.0);
            glVertex3d(imagePos[i+1][j+1].x, imagePos[i+1][j+1].y, imagePos[i+1][j+1].z);  //空間に点を配置
            glColor4d(imageCol[i+1][j].x, imageCol[i+1][j].y, imageCol[i+1][j].z, 1.0);
            glVertex3d(imagePos[i+1][j].x, imagePos[i+1][j].y, imagePos[i+1][j].z);  //空間に点を配置
        }
    }
    glEnd();
    ...
    for (int i=0; i<GL_WIN_TOTAL; i++) {
        ...
        double depth = pointPos[i].Z;
        if (depth<100 || depth>depthMax) depth = depthMax;
        
        depthImage.at<float>(N, M) = depth;  //生データ深度画像
        depthDispImage.at<unsigned char>(N, M) = MAX(0, (depthMax-depth)/depthMax*255);  //表示用深度画像
    }
    cv::imshow("DEPTH", depthDispImage);
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    switch (key) {
        ...
        case 's':
            fp = fopen("scandata.txt", "w");
            for (int i=0; i<GL_WIN_TOTAL; i++) {
                fprintf(fp, "%f,%f,%f\n", pointPos[i].X, pointPos[i].Y, pointPos[i].Z);
            }
            fclose(fp);
            break;
