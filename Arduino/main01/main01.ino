...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    int separator_index = received_data.indexOf(','); 

    if (separator_index > 0) {
      String x_string = received_data.substring(0, separator_index);
      String y_string = received_data.substring(separator_index + 1);
      
      target_x = x_string.toInt();
      target_y = y_string.toInt();
      
      Serial.println("Received data: x = " + String(target_x) + ", y = " + String(target_y));
    }
  }
