...
void display()
{
    ...
    // 最大の輪郭を探すための変数を初期化
    int maxAreaIndex = -1;
    double maxArea = 70.0; // 面積のしきい値 (これより小さいノイズは無視)


    // 全ての輪郭をチェックして、一番大きいものを探す
    for (int i = 0; i < contours.size(); i++)
    { 
        double area = contourArea(contours[i]); // 輪郭の面積を計算
        
        // 現在の最大値よりも大きく、かつノイズ(70)より大きい場合、記録更新
        if (area > maxArea)
        { 
            maxArea = area;
            maxAreaIndex = i; // インデックスを保存
        }
    }

    // 有効な物体（最大の輪郭）が見つかった場合のみ処理を実行
    if (maxAreaIndex != -1) 
    {
        // 輪郭のモーメントを計算して、重心を求める
        cv::Moments m = cv::moments(contours[maxAreaIndex]);
        if (m.m00 != 0) // ゼロ除算防止
        {
            ...
            // 認識された輪郭を緑色で描画（視覚的な確認用）
            cv::drawContours(scanImage, contours, maxAreaIndex, cv::Scalar(0, 255, 0), 2);

            // 画面上の2Dピクセル座標を、OpenGLの世界座標(cm)に逆変換する
            Vec_3D pointF;
            // binImage.rows - p.y で上下反転していることに注意
            gluUnProject(p.x, binImage.rows - p.y, 0, model, proj, view, &pointF.x, &pointF.y, &pointF.z);
            
            footPoints.push_back(pointF); // 変換後の実世界座標を格納
            
            printf("Detected X: %.2f, Y: %.2f (Area: %.0f)\n", pointF.x, pointF.y, maxArea); // (デバッグ用)
        }
    }
